        ...
        if (!select.hasClass('outtaHere')) {
            ...
            select.find('option').each(function (index) {
                ...
                _opt.children('a').click(function () {
                    ...
                    $(select.find('option').get(index)).attr('selected', 'selected');
...
$.fn.initializeInputs = function () {
    $(this).find('select').customSelect();
    $(this).find('input:file').customFileInput();

    $(this).find('.datetime').datetimepicker();

    $(this).find('.colorSelector2').each(function () {
        var $self = $(this);
        var _color = $(this).find('div').css('backgroundColor');
        if (_color.indexOf('rgb') != -1) _color = rgb2hex(_color);
        $(this).ColorPicker({
            color:_color,
            ...
            onHide:function (colpkr) {
                $self.children().css('backgroundColor', '#' + hex);
                $(colpkr).fadeOut(500);
                return false;
            },
            onChange:function (hsb, hex, rgb) {
                $self.children().css('backgroundColor', '#' + hex);
                var param = $self.children().data('param');
                $('input[type=hidden][name=' + param + ']').val(hex);
            }
        });
    });
    ...
    if (typeof $().simpleLightbox == 'function') {
        ...
        $(this).find('.popup-button').simpleLightbox({
            faderOpacity:0.9,
            duration:200,
            faderBackground:'#aaa',
        ...
        $(this).find('.ajax-popup-button').simpleLightbox({
            faderOpacity:0.9,
            duration:200,
            faderBackground:'#aaa',
            closeLink:'a.close, a.close-btn',
            ...
            ajaxSuccess: function(){
                $('.popup:visible').initializeInputs();
            }
        });

        $(this).find('a.but').simpleLightbox({
            duration:200,
            faderBackground:'#fff',
            closeLink:'a.close, a.close-btn',
