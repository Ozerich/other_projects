...
define("DB_HOST", 'mysql.madeinmed.z8.ru');
define("DB_USER", 'dbu_madeinmed_4');
define("DB_PASSWORD", 'eNt9du8tFGK');
define("DB_NAME", 'db_madeinmed_25');
...
$parsers = array(
    new ImidzSummerParser(), new ImidzWinterParser(), new KamtehParser(), new LasertaParser(), new KuperParser(), new Letozima4WinterParser()
);
...
if(isset($_GET['save'])){
	...
	foreach($result as $i => &$line){
		$line[] = isset($_POST['type'][$i]) && $_POST['type'][$i] == 1 ? 'На складе' : 'В наличии';
	}
...
if (isset($_GET['ajax'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'add':
			...
			$is_winter = isset($_POST['type']) && $_POST['type'] == 'winter' ? 1 : 0; 

            if ($new_brand == '') {

                if ($model == '0') {
                    $model = $book->addModel($brand, $new_model, $spikes, $is_winter);
...
if ($_FILES) {
    ...
    foreach ($result as $tyre) {
        fputcsv($f, array(
            $tyre->file, $tyre->brand, $tyre->model, $tyre->index, $tyre->width, $tyre->height, $tyre->diametr, $tyre->season, $tyre->spikes, $tyre->price, $tyre->count, $tyre->in_stock ? 'На складе' : 'В наличии'
        ));
    }
    ...
    <div class="form">
        ...
        <form action="index.php?names" method="post" enctype="multipart/form-data">
            <div class="parser-item">
                <label for="names_file">Справочник имен<br/><a href="index.php?names_csv" target="_blank">Скачать CSV</a></label>
	...
	<form action="index.php?save" method="post">
		<div class="table">
			...
			<div class="header">
				...
				<input type="submit" onclick="document.location.href='<?=$filename?>';; return false;" value="Сохранить в CSV"/>
				<br clear="all"/>
			</div>
			<table>
				<thead>
