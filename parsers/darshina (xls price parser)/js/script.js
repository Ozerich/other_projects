    ...
    $.getJSON('index.php?ajax&action=data', function (data) {
            ...
            for (var bi in data) {
                if (data[bi].name.toLowerCase() == brand.toLowerCase()) {
                    ...
                    for (var mi in data[bi].models) {
                        models.find('option').last().after('<option spikes="' + (data[bi].models[mi].spikes != '0' ? '1' : '0') + '" value="' + data[bi].models[mi].id + '">' + data[bi].models[mi].name + '</option>')
                    }
        ...
        $('.form-add .manufactures').each(function () {
            ...
            $(this).on('change', function () {
                ...
                for (var i in models) {
                    select.find('option').last().after('<option value="' + models[i].id + '">' + models[i].name + '</option>');
                }
            });
        })
