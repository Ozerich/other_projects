...
$calculator_results = isset($_SESSION['calculator_results_3']) ? $_SESSION['calculator_results_3'] : array();
for ($i = 1; $i <= 2; $i++) {
...
if (!isset($_SESSION['selected_type_3'])) {
    $_SESSION['selected_type_3'] = 0;
}

$selected_type = isset($_SESSION['selected_type_3']) ? $_SESSION['selected_type_3'] : 0;
...
<br/><br/>
            Рады предложить вам новый сервис «Выбор налогового режима» - программу, которая позволит торговой компании сравнить общую сумму платежей в бюджет в 2013 году при различных режимах налогообложения по заданным параметрам. Кроме того, справочно выдается информация о сумме налогов и взносов при тех же показателях, но для тарифов 2012 года. Результат - возможность оптимизировать налоговую нагрузку.
            <br/><br/>
            ...
            С уважением, редакция журнала «Учет торговле»
