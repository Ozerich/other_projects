function ViewModel(type) {
    ...
    this.var_1_3 = ko.observable(1.569);
    this.var_1_4 = ko.observable(iv('var_1_4'));
    ...
    this.step1_custom = ko.computed(function(){
        ...
        var d = this.var_1_4() ? parseFloat(this.var_1_4()) : 0;

        return Math.round(a * b * 12 * c * d * 0.15);
    ...
    this.ins_c3 = ko.observable(cv('ins_c3'));
    ...
    this.ins_var_3 = ko.computed({
        'read':function () {
            var res = that.ins_var_2() !== '' ? that.ins_var_2() * (that.ins_c3() ? 0.2 : 0.22) : '';
    ...
    this.ins_var_4 = ko.computed({
        'read':function () {
            var res = that.ins_var_2() !== '' ? that.ins_var_2() * (that.ins_c3()  ? 0 : 0.029) : '';
    ...
    this.ins_var_6 = ko.computed({
        'read':function () {
            var res = that.ins_var_2() !== '' ? that.ins_var_2() * (that.ins_c3()  ? 0 : 0.051) : '';
    ...
    this.var_4_1 = ko.observable(iv('var_4_1'));
    this.var_4_2 = ko.computed(function(){


        var res = num(this.var_4_1()) + this.ins();

        return Math.min(res, num(num(this.step1_result()) * 0.5));
    }, this);

    this.step4_result = ko.computed(function(){
        return this.step1_result() - this.var_4_2();
    }, this);


    this.result = ko.computed(function () {

        return Math.round(this.step4_result() + (this.nds() ? num(this.nds()) : 0) + this.ins());
    ...
    this.prev_year_result = ko.computed(function () {
        ...
        if (!num(this.ins_is_manual())) {
            var var_3 = this._ins_var_2() * (this.ins_c3() ? 0.2 : 0.22);
            var var_4 = this._ins_var_2() * (this.ins_c3() ? 0 : 0.029);
            var var_6 = this._ins_var_2() * (this.ins_c3() ? 0 : 0.051);
        ...
        var res = this.result() - this.ins() + ins;

        if(parseInt(this.step1_is_manual()) == 0)
        {
            var step_1 = parseFloat(this.var_1_1()) * parseFloat(this.var_1_2()) * parseFloat(this.var_1_4()) * 12 * 0.15 * 1.4942;

            var var_4_2 = Math.min(parseFloat(this.var_4_1().length ? this.var_4_1() : 0) + ins, step_1 * 0.5);

            var step_4 = step_1 - var_4_2;

            res = res - this.step4_result() + step_4;
        }
